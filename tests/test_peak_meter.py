  ...
  <project properties='properties;' metadatas='metadatas, author=(string)Author, scaled_proxy_width=(int)0, scaled_proxy_height=(int)0, render-scale=(double)100, format-version=(string)0.7;'>
    ...
    <timeline properties='properties, auto-transition=(boolean)true, snapping-distance=(guint64)6420600;' metadatas='metadatas, markers=(GESMarkerList)&quot;EMPTY&quot;, duration=(guint64)2456000000;'>
      ...
      <groups>
        ...
        def bus_message_cb(unused_bus, message):
            if message.get_structure().get_value("peak") is not None:
                mainloop.quit()

        def begin_playback():
            pipeline.play()
            pipeline.get_bus().connect("message::element", bus_message_cb)
            return False
        ...
        GLib.timeout_add(500, begin_playback)
        mainloop.run(timeout_seconds=10)

        left_peak = editorperspective.viewer.left_peakmeter.peak
        right_peak = editorperspective.viewer.right_peakmeter.peak
        self.assertGreater(0, left_peak)
        self.assertGreater(0, right_peak)
        self.assertGreater(left_peak, MIN_PEAK)
        self.assertGreater(right_peak, MIN_PEAK)
