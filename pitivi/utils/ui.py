...
def format_framerate(framerate):
    """Formats the framerate of for display."""
    ...
    if framerate.num == 0 or framerate.denom == 0:
        ...
        return "0 fps"
    return "{0:.5n} fps".format(framerate.num / framerate.denom)
...
def format_audiorate(rate):
    ...
    return "{0:.5n} kHz".format(rate / 1000.0)
...
def format_audiochannels(channels):
    ...
    unique_vals = {"1": _("Mono"), "2": _("Stereo")}

    try:
        return unique_vals[str(channels)]
    except KeyError:
        return "{0:n}".format(channels)
...
def format_pixel_aspect_ratio(ratio):
    ...
    try:
        return unique_vals[(ratio.num, ratio.denom)]
    except KeyError:
