...
class PluginPlaceHolder(Gtk.Box):
    ...
    def __init__(self):
        ...
        label = Gtk.Label.new(_('<span size="x-large">No plugins available</span>'))
        label.set_use_markup(True)
        label.props.margin_top = PADDING * 10
        label.set_halign(Gtk.Align.CENTER)
        description = Gtk.Label.new(_("Plugins can be installed at a user or system level. To add them, you have to drop them in the corresponding plugin directory."))
        ...
        user_button = Gtk.Button(_("User directory"))
        user_button.connect('clicked', self.__open_directory_clicked,
                            get_user_plugins_dir())

        system_button = Gtk.Button(_("System directory"))
