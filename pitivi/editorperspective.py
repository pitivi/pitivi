...
class EditorPerspective(Perspective, Loggable):
    ...
    def _create_ui(self):
        ...
        self.intro = IntractiveIntro(self.app)
    ...
    def intractive_intro_start(self, unused_widget):
        # pylint: disable=attribute-defined-outside-init
        if not self.intro.is_intro_running:
            #start intro
            self.intro.start_tour(unused_widget)
            self.intro.is_intro_running = True
        else:
            #stop intro
            self.intro.stop_tour(unused_widget)
            self.intro.is_intro_running = False
