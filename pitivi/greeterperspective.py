...
class GreeterPerspective(Perspective):
    ...
    def refresh(self):
        ...
        if children:
            self.__topvbox.remove(children[0])
        ...
        if recent_items:
            ...
            self.__topvbox.pack_start(self.__recent_projects_vbox, False, False, 0)
            self.__recent_projects_listbox.show_all()
        else:
            self.headerbar.set_title("Pitivi")
            self.__topvbox.pack_start(self.__welcome_vbox, False, False, 0)
