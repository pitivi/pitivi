...
class GreeterPerspective(Perspective):
    ...
    def __update_headerbar(self):
        # The three states of the greeter.
        # 1. welcome - recent projects are not present
        # 2. projects - recent projects and search entry are both present
        # 3. selection - recent projects are present but search entry is not present
        welcome = not bool(self.__recent_items)
        projects = bool(self.__recent_items) and self.__search_entry.get_visible()
        selection = bool(self.__recent_items) and not self.__search_entry.get_visible()
