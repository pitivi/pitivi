...
class ProjectManager(GObject.Object, Loggable):
    ...
    def _try_using_backup_file(self, uri):
        backup_path = self._make_backup_u_r_i(path_from_uri(uri))
        use_backup = False
        try:
            ...
            if time_diff > 0:
                ...
                if use_backup:
                    uri = self._make_backup_u_r_i(uri)
    ...
    def load_project(self, uri):
        ...
        project.connect_after("missing-uri", self._missing_u_r_i_cb)
    ...
    def save_project(self, uri=None, formatter_type=None, backup=False):
        ...
        if backup:
            if self.current_project is not None and self.current_project.uri is not None:
                ...
                uri = self._make_backup_u_r_i(self.current_project.uri)
            else:
    ...
    def _clean_backup(self, uri):
        if uri is None:
            return
        path = path_from_uri(self._make_backup_u_r_i(uri))
        if os.path.exists(path):
            os.remove(path)
            self.debug('Removed backup file: %s', path)

    def _make_backup_u_r_i(self, uri):
    ...
    def _missing_u_r_i_cb(self, project, error, asset):
