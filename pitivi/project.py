...
class Project(Loggable, GES.Project):
    ...
    def __get_thumb_from_assets_cache(self, assets_uri):
        """Gets project thumbnail from assets thumbnail cache."""
    ...
    def create_thumb(self):
        ...
        if not assets_uri:
            # User deleted all the assets, so remove the
            # existing project thumbnails, if any.
        ...
        if normal_thumb_path or large_thumb_path:
            ...
            # Let's try out the assets thumbnail cache.
            thumb = self.__get_thumb_from_assets_cache(assets_uri)
            if not thumb:
                return
