...
class TimelineContainer(Gtk.Grid, Zoomable, Loggable):
    ...
    def __action_search_cb(self, unused_action, unused_param):
        ...
        win.connect("key-press-event", win.entry_key_press_event_cb)
        win.entry.connect("changed", win.entry_changed_cb)
        win.results_treeview.connect("row-activated", win.row_activated_cb)

        win.vbox.pack_start(win.entry, True, True, 0)
        win.vbox.pack_start(win.results_window, True, True, 0)
