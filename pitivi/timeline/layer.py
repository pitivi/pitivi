...
class LayerControls(Gtk.EventBox, Loggable):
    ...
    def __init__(self, ges_layer, app):
        ...
        self.video_track_hide_button = Gtk.ToggleButton(
            None, image=Gtk.Image.new_from_icon_name("video-x-generic", Gtk.IconSize.BUTTON))
        self.video_track_hide_button.set_active(True)
        self.video_track_hide_button.connect("toggled", self._hide_video_cb)
        name_row.pack_start(self.video_track_hide_button, False, False, 0)
    ...
    def update(self, media_types):
        ...
        if media_types & GES.TrackType.VIDEO or not media_types:
            ...
            # Disable video track hide button.
            self.video_track_hide_button.set_sensitive(False)
    ...
    def _hide_video_cb(self, button):
        ...
        if not button.get_active():
            # Hide video track(s)
            self.ges_layer.set_active_for_tracks(active=False, tracks=tracks)
        else:
            # Show video track(s)
            self.ges_layer.set_active_for_tracks(active=True, tracks=tracks)
