...
class LayerControls(Gtk.EventBox, Loggable):
    ...
    def __init__(self, ges_layer, app):
        ...
        self.__update_mute_button_status_cb(self.ges_layer, False, [])
        ...
        self.ges_layer.connect("active-changed", self.__update_mute_button_status_cb)
    ...
    def __update_mute_button_status_cb(self, ges_layer, active, tracks):
        for audio_track in self.timeline_audio_tracks:
            if self.ges_layer.get_active_for_track(audio_track):
                active = True

        self.mute_toggle_button.set_active(not active)
        icon_name = "audio-volume-high-symbolic" if active else "audio-volume-muted-symbolic"
