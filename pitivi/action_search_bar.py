...
class ActionSearchBar(Gtk.Window):
    ...
    def create_entry(self):
        list_model = Gtk.ListStore(str, str, Gio.SimpleAction, object)
        ...
        self.entry.connect("changed", self._changed_entry_cb)
    ...
    def filter_func(self, model, row, data):
        ...
        if keyword != '':
            ...
            all_found = True
            for key_term in keyword.lower().split(" "):
                found = False
                for term in model[row][3]:
                    if term.startswith(key_term):
                        found = True
                        break
                if not found:
                    all_found = False
    ...
    def _changed_entry_cb(self, *args):
