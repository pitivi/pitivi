    ...
    def entry_suggestion(self):
        ...
        for i in self.app.shortcuts.group_actions:
            for action, title, action_object in self.app.shortcuts.group_actions[i]:
                ...
                store.append([title, accelerator_parsed.accelerator_key, accelerator_parsed.accelerator_mods, action_object])

        renderer = Gtk.CellRendererAccel()
        completion.pack_end(renderer, True)

        completion.set_model(store)
        completion.add_attribute(renderer, "accel_key", 1)
        completion.add_attribute(renderer, "accel_mods", 2)
