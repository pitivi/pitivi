...
class ViewerContainer(Gtk.Box, Loggable):
    ...
    def _createUi(self):
        ...
        marker.connect("button-press-event", self.__marker_button_pressed_cb)
        marker.connect("button-release-event", self.__marker_button_released_cb)
    ...
    def __marker_button_pressed_cb(self, unused_widget, event):
        if event.button == 1:
            ...
            vpane_pos = self.app.gui.editor.toplevel_widget.get_position()
            self.__y_translation = event.y_root - vpane_pos

    def __marker_button_released_cb(self, unused_widget, unused_event):
        self.__y_translation = None

    def __marker_drag_cb(self, unused_widget, event):
        if self.__y_translation:
            vpane = self.app.gui.editor.toplevel_widget
            vpane.set_position(event.y_root - self.__y_translation)
