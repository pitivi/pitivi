...
class ViewerContainer(Gtk.Box, Loggable):
    ...
    def _createUi(self):
        ...
        margin = 3
        marker_length = space * lines
        marker.set_size_request(marker_length, marker_length)
        marker.set_halign(Gtk.Align.START)
        marker.set_margin_left(margin)
        marker.set_margin_bottom(margin)
        ...
        marker.connect("button-press-event", self.__marker_button_press_cb, marker_length + margin)
    ...
    def __marker_button_press_cb(self, unused_widget, event, marker_length):
        if event.button == 1:
            ...
            # Y coordinate of bottom left corner of viewer
            # container w.r.t screen coordinate system.
            y_screen = event.y_root - event.y + marker_length
            # Y coordinate of bottom left corner of viewer
            # container w.r.t mainwindow coordinate system.
            y_mainwindow = self.app.gui.editor.toplevel_widget.get_position()
            self.__y_translation = y_screen - y_mainwindow

    def __marker_button_release_cb(self, unused_widget, unused_event):
        self.__y_translation = None

    def __marker_drag_cb(self, unused_widget, event):
        if self.__y_translation:
            hpaned = self.app.gui.editor.mainhpaned
            vpaned = self.app.gui.editor.toplevel_widget

            # Resizing the height of viewer container.
            vpaned.set_position(event.y_root - self.__y_translation)

            aspect_frame_height = self.target.get_allocation().height
            # Viewer container's width for the new height.
            vc_width = int(aspect_frame_height * self.target.props.ratio) + 2 * SPACING
            mainwindow_width = self.app.gui.get_size()[0]
            hpaned.set_position(mainwindow_width - vc_width)
