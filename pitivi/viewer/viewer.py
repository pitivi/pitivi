...
class ViewerContainer(Gtk.Box, Loggable):
    ...
    def __init__(self, app):
        ...
        self.__cursor = None
        self.__corner_click_pos = numpy.array([])
    ...
    def __marker_button_press_cb(self, widget, event):
        if event.button == 1:
            self.__corner_click_pos = numpy.array([event.x, event.y])
    ...
    def __marker_drag_cb(self, unused_widget, event):
        if self.__corner_click_pos.size:
            curr_position = numpy.array([event.x, event.y])
            translation = self.__corner_click_pos - curr_position
            ...
            vpaned.set_position(vpaned.get_position() - translation[1])
