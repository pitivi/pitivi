...
from pitivi.intro_info_list import intro_info_list as  imported_intro_list
...
class IntractiveIntro(GObject.Object):
    ...
    def intractive_intro_start_tour(self, unused_widget):
    ...
    def intractive_intro_stop_tour(self, unused_widget):
    ...
    def start_next_intro(self):
        if self.current_intro_group_index == 0:
            ...
            self.app.gui.editor.is_intro_running = False
            self.intractive_intro_stop_tour(None)
    ...
    def headerbar_overview(self):
        ...
        self.intro_text_list = imported_intro_list[0:8]
    ...
    def viewer_overview(self):
        if len(self.widgits_list) == 0:
            ...
            self.widgits_list.extend(widgits_list)
        self.intro_text_list = imported_intro_list[8:17]
        self.gobject_timeout = GObject.timeout_add(1500, self.intractive_overview_show_popup)
    ...
    def timeline_overview(self):
        if len(self.widgits_list) == 0:
            ...
            self.widgits_list.extend(widgits_list)
        self.intro_text_list = imported_intro_list[17:23]
        self.gobject_timeout = GObject.timeout_add(1500, self.intractive_overview_show_popup)
    ...
    def medialibrary_overview(self):
        ...
        if len(self.widgits_list) == 0:
            ...
            self.widgits_list.extend(widgits_list)
        self.intro_text_list = imported_intro_list[23:28]
        self.gobject_timeout = GObject.timeout_add(1500, self.intractive_overview_show_popup)
    ...
    def effectlist_overview(self):
        if len(self.widgits_list) == 0:
            ...
            self.widgits_list.extend(widgits_list)
        self.intro_text_list = imported_intro_list[28:39]
        self.gobject_timeout = GObject.timeout_add(1500, self.intractive_overview_show_popup)

    def intractive_overview_show_popup(self):
        if self.intractive_intro_index == len(self.widgits_list):
            self.intro_popover.popdown()
            self.current_intro_group_index += 1
            self.emit("run-next-intro", None)
            return False
        self.intro_label.set_markup('<span><b>\n'+str(self.intro_text_list[self.intractive_intro_index])+'</b></span>')
        self.intro_popover.set_relative_to(self.widgits_list[self.intractive_intro_index])
